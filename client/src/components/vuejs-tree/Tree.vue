...
export default {
  ...
  methods: {
    ...
    recFindNode(nodeId, nodes, depth, maxDepth) {
      ...
      nodes.forEach((node) => {
        ...
        if (nodeId == node.id && maxDepth >= depth) {
          ...
          node.nodes &&
          maxDepth > depth &&
          (tmp = _this.recFindNode(nodeId, node.nodes, depth + 1, maxDepth)) !=
            null
        ) {
          ret = tmp;
          return false;
    ...
    onNodeSelected(nodeSelected) {
      ...
      if (this.selectedNode == null && nodeSelected.selected === true) {
        this.selectedNode = nodeSelected;
      } else if (this.selectedNode != null && nodeSelected.selected === false) {
        this.selectedNode = null;
      } else if (this.selectedNode != null && nodeSelected.selected === true) {
        const arrIds = this.findNodePath(
          this.selectedNode.id,
          this.selectedNode.depth
        );
        ...
        this.$nextTick(() => {
          if (_this.selectedNode) {
            const selectArrIds = _this.findNodePath(
              _this.selectedNode.id,
              _this.selectedNode.depth
            );
      ...
      let fn = null;
      if (
        this.options.treeEvents.selected &&
        this.options.treeEvents.selected.state === true
      ) {
    ...
    onNodeExpanded(node, state) {
      ...
      if (
        state === true &&
        this.options.treeEvents.expanded &&
        this.options.treeEvents.expanded.state === true
      ) {
        fn = this.customOptions.treeEvents.expanded.fn;
      } else if (
        this.options.treeEvents.collapsed &&
        this.options.treeEvents.collapsed.state === true
      ) {
    ...
    callSpecificChild(arrIds, fname, args) {
      for (let id of arrIds) {
        const child = this.$refs["tree-row-" + id];
        if (child) {
          child[fname](args);
        }
      }
    ...
    recCollapseAllNodes(arrIds) {
      for (let id of arrIds) {
        this.findNode(id).expanded = false;
      }
    },
    collapseAllNodes() {
    ...
    recGetNodesData(argWanted, conditions, nodes) {
      ...
      nodes.forEach((node) => {
        if (
          node.value &&
          Object.keys(node).filter((key) => conditions[key] === node[key])
        ...
        arr = arr.concat(
          _this.recGetNodesData(argWanted, conditions, node.nodes)
        );
      });
      return arr;
    },
    recGetNodesDataWithFormat(argWanted, conditions, nodes) {
      ...
      nodes.forEach((node) => {
        if (
          node.value &&
          Object.keys(node.value).filter((key) => conditions[key] === node[key])
            .length === Object.keys(conditions).length
        ) {
          arr[node.id] = _this.recGetNodesDataWithFormat(
            argWanted,
            conditions,
            node.nodes
          );
          ...
          Object.assign(
            arr,
            _this.recGetNodesDataWithFormat(argWanted, conditions, node.nodes)
          );
        }
      });
      return arr;
